...
interface FileUploadProps {
  ...
  imageUrls?: string // Update the type to string
}

export const FileUpload: React.FC<FileUploadProps> = ({ onFileUpload, imageUrls, ...props }) => {
  const { id } = useUser() || {}
  ...
  useEffect(() => {
    const urlsArr = imageUrls && imageUrls.split(',')
    urlsArr && setFilePreviews(urlsArr)
  }, [])

  return (
    <div
      className={`grid grid-cols-2 grid-rows-auto gap-x-4 gap-y-3 px-2 py-1 mb-4 ${isDragging && 'shadow-[0_0px_0px_5px_rgba(135,211,248,0.5)_inset]'}`}
    >
      <div
      ...
      {filePreviews.map((preview, index) => (
        <div
          key={index}
          className="size-[130px] relative flex flex-wrap p-2 border-[1px] rounded-md border-gray-300"
        >
          <img
            src={preview}
